...
def animate(i):
	...
	if (numpy.size(Maxs[0,]) > 1 ):
		...
		if (t>501):
			...
			v_g = Vg(signalprec, Maxs, v_g)
			
			# On sauvegarde l'enveloppe actuelle pour l'instant suivant
			signalprec = signalint
			signalint = Maxs
			
			print("Vitesse de groupe à t = ", t-1, " : ", v_g)
			
			VgTheo = sqrt(1 - (W[0,0]/koi)**2)
			
			print("vitesse de groupe theorique : ", VgTheo)
			
		if (t == 500):
			# A la première itération, on sauvegarde le premier signal 
			signalprec = Maxs
			
		if (t == 501):
			# A la seconde itération, on sauvegarde le signal auquel on veut la vitesse de groupe
			signalint = Maxs 
